...
// Proxy to FRED: /fred/series/observations -> https://api.stlouisfed.org/fred/series/observations
...
module.exports = async (req, res) => {
  try {
    ...
    console.log('fred/observations incoming', { method: req.method, params });
    ...
    if (String(params['__test']) === '1') {
      return res.status(200).json({ debug:true, msg:'dummy fredObservations (test mode)', series_id_received: params.series_id || null });
    }
    ...
    const url = `${FRED_BASE}${'/fred/series/observations'}?${buildQuery(params)}`;
    return forwardGET(res, url);
  } catch (err) {
    console.error('fred/observations internal error', err && err.stack);
